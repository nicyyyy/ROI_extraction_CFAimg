...
S=(0.1*double(I)+0.8*double(O)+0.1*double(C));
...
%%
S = floor((S-min(min(S)))*255/(max(max(S))-min((min(S)))));
% S = floor(255*S/(max(max(S))));
level = get_th(S);
[m,n] = size(S);
for i = 1:m
   for j = 1:n
...
figure(10)
imshow(uint8(S))
[xMin, xMax, yMin, yMax] = get_rect4(S);
x = round((xMin+xMax)/2);
y = round((yMin+yMax)/2);
w = round(xMax - xMin);
h = round(yMax - yMin);
...
for i =1:mm
    for j = 1:nn
    grabimg(i,j) = img1(yMin+i-1,xMin+j-1);
    end
end
figure(5)
